    ...
    )
{
  ...
  for (totalbins = 0;  totalbins < *NbrListLen;  totalbins += numbins) {
    ...
    for (_bincnt = 0;  _bincnt < 4 && nbrid < *NbrListLen;  _bincnt++, nbrid += 8) {
      ...
      int binIndex = startoff + (_bincnt << (3 + BIN_SHIFT));
      int tidmask = tid & 15;
    ...
    for (bincnt = 0; bincnt < stopbin; bincnt+=BIN_SIZE) {
      ...
      for (i = 0;  i < BIN_DEPTH;  i++) {
        ...
        aq = AtomBinCache[off + 3];
        if (0.f == aq) 
           break;  /* no more atoms in bin */

        dx = AtomBinCache[off    ] - x;
        dz = AtomBinCache[off + 2] - z;
        dxdz2 = dx*dx + dz*dz;
        dy = AtomBinCache[off + 1] - y;
        r2 = dy*dy + dxdz2;
