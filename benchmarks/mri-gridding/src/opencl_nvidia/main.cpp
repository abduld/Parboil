 ...
 ************************************************************/
pb_Context* setParameters(FILE* file, parameters* p, struct pb_Parameters *prms){
  ...
  pb_context = pb_InitOpenCLContext(prms);
  if (pb_context == NULL) {
...
int main (int argc, char* argv[]){
  ...
  pb_Context *pb_context = setParameters(uksfile_f, &params, prms);
  cl_int ciErrNum;
  cl_device_id clDevice = (cl_device_id) pb_context->clDeviceId;
  cl_platform_id clPlatform = (cl_platform_id) pb_context->clPlatformId;
  cl_context clContext = (cl_context) pb_context->clContext;
...
*/
  ...
  if (workItemDimensions == 0) {
    workItemDimensions = 3;
  }
  
  size_t workItemSizes[workItemDimensions];
  for (int i = 0; i < workItemDimensions; i++) workItemSizes[i] = 0;
