...
void scanLargeArray( unsigned int gridNumElems, cl_mem data_d, cl_context clContext, cl_command_queue clCommandQueue, const cl_device_id clDevice, size_t *workItemSizes) {
    ...
    free(source);
    cl_int clCompilerErr = clBuildProgram(scanLargeArray_program, 1, &clDevice, compileOptions, NULL, NULL); 
    // OCL_ERRCK_VAR(clCompilerErr);
    if (clCompilerErr != CL_SUCCESS) {
      size_t string_size = 0;
      clGetProgramBuildInfo(scanLargeArray_program, clDevice, CL_PROGRAM_BUILD_LOG, 
                            0, NULL, &string_size);
