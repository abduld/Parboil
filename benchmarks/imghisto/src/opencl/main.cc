                  ...
                  cl_command_queue clCommandQueue, cl_kernel clKernel)
{
  ...
  int shmem_bytes = R * BINSp * sizeof(int);
  ...
  clStatus = clSetKernelArg(clKernel,5,sizeof(int),&BINSp);
  clStatus = clSetKernelArg(clKernel,6,sizeof(int),&R);
...
int main(int argc, char **argv) {
  ...
  const char* clSource[] = {readFile("src/opencl/kernel.cl")};
  cl_program clProgram = clCreateProgramWithSource(clContext,1,clSource,NULL,&clStatus);
  CHECK_ERROR("clCreateProgramWithSource")

  char clOptions[50];
  sprintf(clOptions,"-I src/opencl");
  ...
  int SIMULTANEOUS_BLOCKS_PER_SM = 1;
  cl_ulong local_mem_size;
  clGetDeviceInfo(clDevice, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(local_mem_size), &local_mem_size, NULL);
  int REP = local_mem_size/(BINSp*sizeof(unsigned int)*SIMULTANEOUS_BLOCKS_PER_SM);
  ...
  printf("Execution configuration: %d blocks of %d threads, %d sub-histograms per block\n", NUM_BLOCKS, THREADS, REP);
  printf("-------------------------------------------------------------------------------\n");
  ...
  for(int i = 0; i < repeat; i++) {
    ...
    histo_R_per_block(d_histo_block, d_DataA, DATA_SIZE, NUM_BLOCKS, THREADS, BINS, BINSp, REP,
