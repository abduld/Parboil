...
int main(int argc, char* argv[]) {
  ...
  unsigned int lmemKBSize = 24;
  // OCL_ERRCK_RETVAL ( clGetDeviceInfo(clDevice, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(cl_ulong), &lmemSize, NULL) );
  if (clGetDeviceInfo(clDevice, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(cl_ulong), &lmemSize, NULL) != CL_SUCCESS) {
    lmemSize = lmemKBSize*1024;
  }
  
  cl_uint workItemDimensions;
  if (clGetDeviceInfo(clDevice, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, sizeof(cl_uint), &workItemDimensions, NULL) != CL_SUCCESS) {
    workItemDimensions = 3;
  }
  
  size_t workItemSizes[workItemDimensions];
  if (clGetDeviceInfo(clDevice, CL_DEVICE_MAX_WORK_ITEM_SIZES, workItemDimensions*sizeof(size_t), workItemSizes, NULL) != CL_SUCCESS) {
    workItemSizes[0] = 1024;
    workItemSizes[1] = 1024;
    workItemSizes[2] = 1024;
  }
  ...
  const char *source_path[4] = {
    "src/opencl_base/histo_prescan.cl",
    "src/opencl_base/histo_intermediates.cl",
    "src/opencl_base/histo_main.cl",
    "src/opencl_base/histo_final.cl"
  };
  char *source[4];
  ...
  input =           clCreateBuffer(clContext, CL_MEM_READ_WRITE, 
      even_width*(((img_height+UNROLL)/UNROLL)*UNROLL)*sizeof(unsigned int), NULL, &ciErrNum); OCL_ERRCK_VAR(ciErrNum);
