...
static void sad4_one_macroblock(unsigned short *blk_sad,
				...
				int frame_x,
				int mb_width,
				int mb_height, int off, unsigned short* base);
      ...
      for (mb_x = 0; mb_x < mb_width; mb_x++)
	{
	  sad4_one_macroblock
	    (blk_sad + (mb_y * mb_width + mb_x) * (SAD_TYPE_7_CT * MAX_POS_PADDED),
	     ...
	     mb_x * 16,
	     mb_width,
	     mb_height, (mb_y * mb_width + mb_x) * (SAD_TYPE_7_CT * MAX_POS_PADDED), blk_sad);
	}
    }
}
  ...
  for (pos_y = -SEARCH_RANGE; pos_y <= SEARCH_RANGE; pos_y++) {
	...
	for (blkx = 0; blkx < 4; blkx++) {
	  ...
	  for (y = 0; y < 4; y++) {
        ...
        // printf ("-- %d : %d\n", ref_y * width + ref_x, (blky * 4 + y) * width + (blkx * 4 + x));
	  ...
	  base[off + MAX_POS_PADDED*(4*blky+blkx) + pos] = sad;
    printf ("## %d : %d\n", off + MAX_POS_PADDED*(4*blky+blkx) + pos, sad);
	}
      }
    }
  }
