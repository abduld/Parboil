...
#ifndef SAD_LOC_SIZE_BYTES
#define SAD_LOC_SIZE_BYTES 2192
#define MAX_POS 1089
#define CEIL_POS 61
    ...
    __global unsigned short* img_ref) //  __read_only image2d_t img_ref)
{
  ...
  if ((lidx >> 4) < (THREADS_W * THREADS_H))
  {
    int ty = (lidx >> 4) / THREADS_W;
    int tx = (lidx >> 4) - mul24(ty, THREADS_W);
    int tgroup = lidx & 15;
  ...
  if ((mb_x < mb_width) && (mb_y < mb_height))
  {
    ...
    int search_pos;
    int search_pos_base = (lidx % CEIL_POS) * POS_PER_THREAD;
    ...
    for (search_pos = search_pos_base;
      ...
      for(int y=0; y<4; y++) {
        ...
        t = img_ref[sx + sy * img_width]; // (read_imageui(img_ref, texSampler, (int2)(search_off_x, search_off_y + y) )).x;
  ...
  if ((lidx >> 5) < (THREADS_W * THREADS_H))
  {
    int tgroup = lidx & 31;
    int ty = (lidx >> 5) / THREADS_W;
    int tx = (lidx >> 5) - mul24(ty, THREADS_W);
    int index;
...
#else
        __global uint2 * _blk_sad = (__global uint2 *) (blk_sad );
        _blk_sad[index] 
          = SAD_LOC_8B_GET(mul24(ty, THREADS_W) + tx, index);
#endif
      }
...
__kernel void larger_sad_calc_8(__global unsigned short *blk_sad,
				  ...
				  int mb_height)
{
  int tx = get_local_id(1) & 1;   // 0 1 0 1 ...
  int ty = get_local_id(1) >> 1;  // 0 0 1 1 ...
  ...
  // MXPA
    bo_4 = (__global unsigned short *) (blk_sad + get_local_id(0));
    bo_5 = (__global unsigned short *) (blk_sad + get_local_id(0));
...
#else
      __global uint* _bi = (__global uint*) (bi);
      uint i00 = _bi[search_pos];
      uint i01 = _bi[search_pos + MAX_POS_PADDED/2];
      uint i10 = _bi[search_pos + 4*MAX_POS_PADDED/2];
      uint i11 = _bi[search_pos + 5*MAX_POS_PADDED/2];

      __global uint* _bo_6 = (__global uint *) bo_6;
      __global uint* _bo_5 = (__global uint *) bo_5;
      __global uint* _bo_4 = (__global uint *) bo_4;
      _bo_6[search_pos]                  = i00 + i10;
      _bo_6[search_pos+MAX_POS_PADDED/2] = i01 + i11;
      _bo_5[search_pos]                  = i00 + i01;
      _bo_5[search_pos+2*MAX_POS_PADDED/2] = i10 + i11;
      _bo_4[search_pos]                  = (i00 + i01) + (i10 + i11);
#endif
    }
}
  ...
  for (; search_pos < (MAX_POS+1)/2; search_pos += 32)
    {
      ...
      uint i00 = _bi[search_pos];
      uint i01 = _bi[search_pos + MAX_POS_PADDED/2];
      uint i10 = _bi[search_pos + 2*MAX_POS_PADDED/2];
      uint i11 = _bi[search_pos + 3*MAX_POS_PADDED/2];

      __global uint* _bo_3 = (__global uint *) bo_3;
      __global uint* _bo_2 = (__global uint *) bo_2;
      __global uint* _bo_1 = (__global uint *) bo_1;
      _bo_3[search_pos]                  = i00 + i10;
      _bo_3[search_pos+MAX_POS_PADDED/2] = i01 + i11;
      _bo_2[search_pos]                  = i00 + i01;
      _bo_2[search_pos+MAX_POS_PADDED/2] = i10 + i11;
      _bo_1[search_pos]                  = (i00 + i01) + (i10 + i11);
