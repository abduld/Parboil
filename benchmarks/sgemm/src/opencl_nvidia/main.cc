...
static cl_int getDevices(cl_program program, size_t* _num_devices, cl_device_id* _devices) {
  cl_int status;
  status = clGetProgramInfo(program, CL_PROGRAM_NUM_DEVICES, sizeof(size_t), _num_devices, NULL);
  assert(*_num_devices < 512&& "Too many devices!");
  status = clGetProgramInfo(program, CL_PROGRAM_DEVICES, sizeof(cl_device_id) * (*_num_devices), _devices, NULL);
  return status;
}

static void printErr(cl_program program) {
  size_t num_devices;
  cl_device_id device_list[512];
  getDevices(program, &num_devices, device_list);
  char buffer[65536];
  std::cout << "--------------------------------------------------------\n";
  for (unsigned int i = 0; i < num_devices; ++i) {
    clGetProgramBuildInfo(program,
      device_list[i], CL_PROGRAM_BUILD_LOG, 65536, buffer, NULL);
    std::cout << "** " << buffer << "**\n";
  }
}
