               ...
               __constant float* dev_binb, int NUM_SETS, int NUM_ELEMENTS)
{
  ...
  unsigned int t_id = get_local_id(0);
  ...
  for(unsigned int w = 0; w < NUM_BINS*NUM_HISTOGRAMS; w += BLOCK_SIZE )
    {
      if(w+t_id < NUM_BINS*NUM_HISTOGRAMS)
        {
  ...
  for(unsigned int i = 0; i < NUM_ELEMENTS; i += BLOCK_SIZE )
    {
      ...
      if( t_id + i < NUM_ELEMENTS )
        { // reading outside of bounds is a-okay
#if 0
          data_s[t_id] = (struct cartesian)
            {data_x[t_id + i], data_y[t_id + i], data_z[t_id + i]};
#else
          data_s[t_id].x = data_x[t_id + i];
          data_s[t_id].y = data_y[t_id + i];
          data_s[t_id].z = data_z[t_id + i];
#endif
        }
      ...
      for(unsigned int j = (do_self ? i+1 : 0); j < NUM_ELEMENTS; 
          j += BLOCK_SIZE)
        {
          ...
          if(t_id + j < NUM_ELEMENTS)
            {
              random_x_s = random_x[t_id + j];
              random_y_s = random_y[t_id + j];
              random_z_s = random_z[t_id + j];
            }
          ...
          for(unsigned int k = 0; 
              ...
              k += 1)
            {
              ...
              warpnum = t_id / (WARP_SIZE/HISTS_PER_WARP);
              if((distance < dev_binb[min]) && (distance >= dev_binb[max]) && 
                 (!do_self || (t_id + j > i + k)) && (t_id + j < NUM_ELEMENTS))
                {
                        atom_inc(&(warp_hists[NUM_HISTOGRAMS * bin_index + warpnum]));
                }
            }
        }
    }
  ...
  warp_index = t_id & ( (NUM_HISTOGRAMS>>1) - 1);
  bin_index = t_id / (NUM_HISTOGRAMS>>1);
          ...
          if(warp_index < offset && bin_base+bin_index < NUM_BINS )
            {
              // unsigned long sum =
              sum =
                warp_hists[NUM_HISTOGRAMS * (bin_base + bin_index) + warp_index] + 
                warp_hists[NUM_HISTOGRAMS * (bin_base + bin_index) + warp_index+offset];
              warp_hists[NUM_HISTOGRAMS * (bin_base + bin_index) + warp_index] = sum;
            }
        }
    }
  ...
  hist_base = histograms + NUM_BINS * bx;
  if(t_id < NUM_BINS)
    {
      hist_base[t_id] = warp_hists[NUM_HISTOGRAMS * t_id + 0];
    }
}
