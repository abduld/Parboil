...
int main(int argc, char** argv) {
  ...
  if (clStatus != CL_SUCCESS) clDeviceProp.major = 2; // CHECK_ERROR("clGetDeviceInfo")
	clStatus = clGetDeviceInfo(clDevice,CL_DEVICE_COMPUTE_CAPABILITY_MINOR_NV,sizeof(cl_uint),&(clDeviceProp.minor),NULL);
  if (clStatus != CL_SUCCESS) clDeviceProp.minor = 0; // CHECK_ERROR("clGetDeviceInfo")
	clStatus = clGetDeviceInfo(clDevice,CL_DEVICE_MAX_COMPUTE_UNITS,sizeof(cl_uint),&(clDeviceProp.multiProcessorCount),NULL);
  if (clStatus != CL_SUCCESS) clDeviceProp.multiProcessorCount = 14; // CHECK_ERROR("clGetDeviceInfo")
