          ...
          float * __restrict__ outR, float * __restrict__ outI)
{
  ...
  // Loop over space and frequency domains.
  // Generally numX > numK.
  // Since the loops are not tiled, it is better that the loop with the
  // smaller cache footprint be innermost.
  for (int indexX = 0; indexX < numX; indexX++) {
    float outRacc = 0.0f;
    float outIacc = 0.0f;

    // Sum contributions to this point over all frequencies
    for (int indexK = 0; indexK < numK; indexK++) {
      ...
      outRacc += rRhoPhi * cosArg - iRhoPhi * sinArg;
      outIacc += iRhoPhi * cosArg + rRhoPhi * sinArg;
    }
    outR[indexX] = outRacc;
    outI[indexX] = outIacc;
  }
