...
__kernel void block2D_hybrid_coarsen_x(float c0,float c1,__global float* A0, __global float* Anext, int nx, int ny, int nz, __local float* sh_A0)
{
	...
	for(int k=1;k<nz-1;k++)
	{
		...
		{
#if 0
      a_up        = y_h_bound ? A0[Index3D(nx,ny,i,j+1,k)] : sh_A0[sh_id+2*get_local_size(0)+0];
      a_down      = y_l_bound ? A0[Index3D(nx,ny,i,j-1,k)] : sh_A0[sh_id-2*get_local_size(0)+0];
			a_left_right=x_l_bound?A0[Index3D (nx, ny, i+(-1), j, k )]:sh_A0[sh_id-1];
		
#else
      if (y_h_bound) a_up         = A0[Index3D(nx,ny,i,j+1,k)];
      else           a_up         = sh_A0[sh_id+2*get_local_size(0)];
      if (y_l_bound) a_down       = A0[Index3D(nx,ny,i,j-1,k)];
      else           a_down       = sh_A0[sh_id-2*get_local_size(0)];
      if (x_l_bound) a_left_right = A0[Index3D(nx,ny,i-1,j,k)];
      else           a_left_right = sh_A0[sh_id-1];
#endif
		...
		{	
#if 0
      a_up        = y_h_bound ? A0[Index3D(nx,ny,i2,j+1,k)] : sh_A0[sh_id2+2*get_local_size(0)+0];
      a_down      = y_l_bound ? A0[Index3D(nx,ny,i2,j-1,k)] : sh_A0[sh_id2-2*get_local_size(0)+0];
			a_left_right=x_h_bound?A0[Index3D (nx, ny, i2+1, j, k )]:sh_A0[sh_id2+1];
#else
      if (y_h_bound) a_up         = A0[Index3D(nx,ny,i2,j+1,k)];
      else           a_up         = sh_A0[sh_id2+2*get_local_size(0)];
      if (y_l_bound) a_down       = A0[Index3D(nx,ny,i2,j-1,k)];
      else           a_down       = sh_A0[sh_id2-2*get_local_size(0)];
      if (x_h_bound) a_left_right = A0[Index3D(nx,ny,i2+1,j,k)];
      else           a_left_right = sh_A0[sh_id2+1];
#endif
